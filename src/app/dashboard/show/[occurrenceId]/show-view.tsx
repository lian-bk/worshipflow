...
export function ShowView({
  ...
  const goToSlide = useCallback(
    (index: number) => {
      ...
      setLiveIndex(index);
      setIsBlank(false);
      sendPayload({ type: "slide", slide: flatSlides[index].slide });
    },
    [flatSlides, sendPayload]
  );
  ...
  }, [sendPayload, churchName, tagline]);
          ...
          <div className="ml-auto flex items-center gap-2">
            {!isElectron ? (
              <span className="text-xs text-slate-400">
                Projector output needs the WorshipFlow desktop app — this browser tab is preview-only.
              </span>
            ) : !projectorOpen ? (
              <>
                <select
